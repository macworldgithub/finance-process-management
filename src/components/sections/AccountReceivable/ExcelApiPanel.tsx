...
import { Button, Select, Input, Upload, message, Radio, Card, Typography } from "antd";
...
const ExcelApiPanel: React.FC = () => {
  ...
  const handleFetchSections = async () => {
    try {
      ...
      const endpoint = mode === "single" ? "/single-table-allowed" : "/multiple-table-allowed";
      const { data } = await apiClient.get(endpoint);
      setSections(data.sections || []);
      setInstructions(data.instructions || []);
      setSelectedSection(undefined);
    } catch (error: any) {
      message.error(error?.response?.data?.detail || "Failed to fetch sections");
    } finally {
  ...
  const handleUploadAll = async () => {
    ...
    try {
      ...
      message.error(error?.response?.data?.detail || "Failed to upload all sections");
    } finally {
      setLoadingAll(false);
    }
  };
  ...
  const handleUploadSingle = async () => {
    ...
    try {
      ...
      message.error(error?.response?.data?.detail || "Failed to upload selected section");
    } finally {
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="flex flex-col gap-2">
            ...
            />
            <label className="text-xs font-medium text-gray-700 mt-2">Excel File</label>
            <Upload {...uploadProps}>
          ...
          <div className="flex flex-col gap-2">
            <label className="text-xs font-medium text-gray-700">Section Name (for single-section upload)</label>
            <Select
              size="small"
          ...
          <div className="flex flex-col gap-2">
            <label className="text-xs font-medium text-gray-700">Backend Instructions</label>
            <TextArea
              readOnly
        ...
        {lastResponse && (
          <div className="mt-2">
            <label className="text-xs font-medium text-gray-700">Last API Response (debug view)</label>
            <TextArea
              readOnly
